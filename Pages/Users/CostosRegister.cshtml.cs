...
{
    public class CostosRegisterModel : PageModel
    {
        ...
        public CostosRegisterModel(UserManager<AppUser> userManager, RoleManager<IdentityRole> roleManager, IToastNotification notify, IMapper mapper)
        {
        ...
        {
            UserInputVM!.OldPassword = UserInputVM.Password;

            if (UserInputVM.Document != null || UserInputVM.Email != null)
            {
                AppUser user = _mapper.Map<AppUser>(UserInputVM);
                user.UserName = UserInputVM!.Email;
                string role = Request.Form["rUserRole"].ToString();
                string roleToAdd = role != "" ? role : RolesApp.Costos;
