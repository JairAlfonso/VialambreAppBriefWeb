...
{
    public class PiezaEditModel : PageModel
    {
        ...
        public PiezaEditModel(UserManager<AppUser> userManager, BriefService briefService, IWebHostEnvironment host, IToastNotification notify)
        {
        ...
        public async Task<IActionResult> OnGetEliminarArchivo(int briefid, int numPieza)
        {
            ...
            if (User.IsInRole("Asesor"))
            {
                ...
                if (archivopieza != null && !string.IsNullOrEmpty(archivopieza.ArchivoPieza))
                {
                    ...
                    await _briefService.UpdatePiezaAsync(archivopieza);

                    // Redirige de vuelta a la página de briefedit
                    _notify.AddWarningToastMessage("Archivo de pieza eliminado!!");
                    return RedirectToPage("/Briefs/Client/PiezaEdit", new { briefid, numPieza });
                }
                else
                {
        ...
        public async Task<IActionResult> OnGetDeleteAndBackAsync(int briefid, int numPieza, int piezaAnt)
        {
            if (User.IsInRole("Asesor"))
            {
                ...
                if (pieza != null)
                {
                    ...
                    return RedirectToPage("/Briefs/Client/PiezaEdit", new { briefid, numPieza = piezaAnt });
                }
                else
                {
                    _notify.AddErrorToastMessage("No se encontró la pieza");
        ...
        public async Task<IActionResult> OnPostFinalizarAsync(int briefid, int numPieza)
        {
            ...
            if (piezaOriginal != null)
            {
                ...
                if (PiezaDetails.Publicidad == false || PiezaDetails.Publicidad == null)
                {
                    ...
                    piezaOriginal.AcabadosyAccesoriosInput = "No requiere";
                }
                ...
                if (string.IsNullOrEmpty(PiezaDetails.PiezaName) || PiezaDetails.CantidadPiezas == null)
                {
                    ...
                    RedirectToPage("/Briefs/Client/PiezaEdit", new { briefid = BriefDetails.BriefId, numPieza = NumeroPieza!.NumeroPieza });
                }
                else
                {
            ...
            else
            {
                ...
                RedirectToPage("/Briefs/Client/BriefReadOnly", new { briefid = BriefDetails!.BriefId });
            }
        ...
        public async Task<IActionResult> OnPostAgregarPiezaAsync(int briefid, int numPieza)
        {
            ...
            if (piezaOriginal != null)
            {
                ...
                if (piezaOriginal.ArchivoPieza == null && PiezaDetails.FilePieza != null)
                {
                    string fileNewName = await FileManager.CopyFilePieza(filePieza!, UploadFolderPieza);
                    piezaOriginal.ArchivoPieza = Path.Combine(PiezaFolder, fileNewName);
                }
                else if (piezaOriginal.ArchivoPieza == null && PiezaDetails.FilePieza == null)
                {
                    piezaOriginal.ArchivoPieza = null;
                }
